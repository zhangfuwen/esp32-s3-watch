...
esp_err_t lvgl_init_system(void)
{
    ...
    ESP_LOGI(TAG, "LVGL buffers allocated: %d bytes each", LVGL_BUF_SIZE * sizeof(lv_color_t));
...
static void lvgl_indev_read_cb(lv_indev_drv_t *drv, lv_indev_data_t *data)
{
    if (cst816_available && cst816_point.pressed) {
        ...
        if (mapped_x >= DISPLAY_WIDTH) mapped_x = DISPLAY_WIDTH - 1;
        if (mapped_y < 0) mapped_y = 0;
        if (mapped_y >= DISPLAY_HEIGHT) mapped_y = DISPLAY_HEIGHT - 1;
